...
for i = m:T
    ...
    Rt = Ft1*data_returns(i) - delta*abs(Ft-Ft1);
    %Calculate Gradient
    A = ((i-1)*A+Rt)/i;
    B = ((i-1)*B+Rt^2)/i;   
    delta_A = A - old_A;
    ...
    for t = 1:i
        if t >1
            DRDFt = -mu*delta*sign(Ft - FtVector(t-1));
            DRDFt1 = mu* data_returns(i)+mu*delta*sign(Ft - FtVector(t-1));
            DFtDw = (1- FtVector(i)^2)*(x + w(m+2)*DFtDwVector(t-1,:)');
        else
            DRDFt = -mu*delta*sign(FtVector(t));
            DRDFt1 = mu* data_returns(i)+mu*delta*sign(Ft);
